...
# Sofascore blocks requests from datacenter IPs (GitHub Actions, and even a
# completely separate cloud sandbox both got a clean 403). Confirmed through
# direct testing that neither signal alone is enough to get past it from a
# datacenter origin — a residential IP alone (plain requests) still got
# blocked 0/5 times from GitHub Actions, and a real Chrome TLS fingerprint
# alone (curl_cffi, no proxy) still got blocked too. Both together, from
# GitHub Actions specifically: 5/5 clean. So both are needed in combination,
# not either on its own — that's why this uses curl_cffi AND a proxy, not one
# or the other, even though earlier attempts (see MIGRATION.md) mistakenly
# concluded first one, then the other, was sufficient alone.
#
# impersonate is pinned to chrome124, not the generic "chrome" alias — that
# generic alias broke TLS verification specifically when combined with this
# proxy (a curl_cffi/proxy interaction bug, not a proxy problem); a pinned
# version sidesteps it. Route through a residential proxy from CI by setting
# SOFASCORE_PROXY_URL; unset locally, where this already works unproxied.
...
def get_json(path: str, retries: int = 3):
    ...
    for attempt in range(retries):
        try:
            r = requests.get(url, headers=HEADERS, proxies=PROXIES, timeout=20, impersonate=IMPERSONATE)
